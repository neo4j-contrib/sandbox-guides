...
Graph Databases offer a natural way of modeling, storing and querying all these types of computer networks in order to improve communication with users, reduce downtime, and save money.
...
We model the physical and virtual components of the network as nodes:
...
We model the connections between these devices as relationships.

We also model the software running on our network as nodes:
...
Our example data is already set up.  You'll find some pointers to help you import your own data in the "resources" section at the end.
...
The data center uses a `10.x.x.x/8` IP address range.
...
Each `Rack` contains different types of `Servers` and has its own `Switch` to connect to the data center routers backplane.
...
For operational simplicity we only run one `Application` per OS which binds to a number of `Ports` on the external network interface.
...
Usually we would get this kind of information from a configuration management database (CMDB), network managment tools or agents installed on the machines.

== Network Exploration: Data Center and Zones

Let's walk through the data, step by step. Let's start with the data center.
...
The data centerconsists of 4 zones, each of which has it's own separate `Network` `10.zone.*/16`, and it's own `Router`.
...
To visualize the `DataCenter` and it's components so far, we can also start at the center and then go 3 hops out.
...
Now we could already have a look at the network connectivity in our data center.
...
Each Rack contains 200 machines of the types we just introduced, so that in total we get 8000 servers in our data center.

As expected, the distribution of the types is inverse to their capabilities.

As the graph visualization of our full data centerwould be pretty but otherwise useless ...
...
We can also query for a distribution of machine types across the data center.
...
Or if we treat our data center as a supercomputer, what's the total amount of CPUs, RAM and disk available:
...
Proper IT infrastructures use a large number of configuration parameters to customize commodity hard- and software for the custom uses.

To manage the volume of interdependent variables Configuration Management Databases (CMDBs) are used to configure and keep track of what is set where.

Systems require certain variables, they can report what is currently configured so that the CMDB software can detect eventual deltas and issue necessary updates.

Security related and other upgrades have to be rolled out across the infrastructure in a coordinated fashion to make sure there is always a

In the past these CMDBs were mostly used for network, hardware and OS level configuration.

Today more and more application level (micro-) services need similar information to get access to internal and external services they depend on.
Those mostly use databases like ZooKepper, Konsul, Eureka, Zipkin? and ?? (HashiCorp?, Pivotal?)

Several of the managed variables are related, especially if they manage (transitive dependencies).
Version conflicts of used dependencies are critical to detect early on.
Although through isolation via containerization this is less of an issue nowadays.

Partially systems like Ansible, Puppet and Chef are used to manage software installation, and update roll outs.

Due to the variety of systems used for providing configuration to the infrastructure, it can be useful to create a unified, up to date view of the situation in your systems graph.
...
* Detect anormal operations like attacks or potential bottlenecks and issue warnings
...
* Lending Club Engineering created a number of network managment projects using Neo4j
...
The data center consists of 4 zones, each of which has it's own separate `Network` `10.zone.*/16`, and it's own `Router`.
