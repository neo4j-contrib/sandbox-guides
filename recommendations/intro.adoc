...
[source, cypher]
----
MATCH (p1:User {name: 'Cynthia Freeman'})-[x:RATED]->(movie)
MATCH (p2:User)-[x2:RATED]->(movie) WHERE p2 <> p1
...
MATCH (p2:User)-[x:RATED]->(movie:Movie) WHERE p2 <> p1
WITH p1, p2, p1Vector, algo.similarity.asVector(movie, x.rating) AS p2Vector
WHERE size(apoc.coll.intersection([x in p1Vector | x.category], [x in p2Vector | x.category])) > 10
RETURN p1.name AS from,
       p2.name AS to,
...
MATCH (u2:User)-[x:RATED]->(movie:Movie) WHERE u2 <> u1

WITH u1, u2, u1Vector, algo.similarity.asVector(movie, x.rating) AS u2Vector
WHERE size(apoc.coll.intersection([x in u1Vector | x.category], [x in u2Vector | x.category])) > 10
