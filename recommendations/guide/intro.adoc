...
Be sure to use the link:https://neo4j.com/docs/cypher-refcard/current/[Cypher Refcard] as a reference for learning Cypher syntax.
...
Let's look at a Cypher query that answers the question "How many reviews does each Matrix review have?". Don't worry if this seems complex, we'll build up our understanding of Cypher as we move along. 

.How many reviews does each Matrix movie have? Click on the block to put the query in the topmost window on the query editor. Hit the triangular icon:play-circle[] button or press kbd:[Ctrl+Enter] to run it and see the resulting visualization.
...
Now let's start generating some recommendations. There are two basic approaches to recommendations algorithms.
...
See this link:https://neo4j.com/graphgist/a7c915c8-a3d6-43b9-8127-1836fecc6e2f[Graph Gist] for another example of using cosine similarity for recommendations.
++++
</div>
++++
...
.Find the users with the most similar preferences as Cynthia Freeman, according to cosine similarity
[source,cypher,subs=attributes]
----
...
* Web link:https://neo4j.com/docs/cypher-refcard/current/[Cypher Refcard]
* Web link:https://neo4j.com/docs/[Neo4j Documentation]
* Blog Post link:https://neo4j.com/blog/collaborative-filtering-creating-teams/[Collaborative Filtering: Creating the Best Teams Ever]
...
* Web link:https://neo4j.com/use-cases/real-time-recommendation-engine/[Solutions: Real-Time Recommendation Engines]
* Book (free download) link:http://graphdatabases.com/[Graph Databases: New Opportunities for Connected Data]
