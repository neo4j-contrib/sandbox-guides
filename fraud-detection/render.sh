...
function render {
$GUIDES/run.sh paysim.adoc index.html +1 "$@"
}

if [ "$1" == "publish" ]; then
	URL=guides.neo4j.com/sandbox/fraud-detection
	render http://$URL -a csv-url=http://guides.neo4j.com/sandbox/fraud-detection/data/ -a env-training
  ...
  render http://$URL
	...
	URL=localhost:8001/sandbox/fraud-detection
